...
| `dictionary.ts`        | `getCharacterData()` loads a character from the `dictionary.char` view + batch-loads component data. `getComponentUses()` finds all characters containing a given component (sorted by Jun Da frequency). `searchCharacters()` searches by character/pinyin/gloss. `getCharacterList()` returns frequency-ordered character lists with pagination.                                              |
